...
class Crop:
    def __init__(self, crop_params, soil_params):
        ...
        self.leaf_growing = True # growing or senescence
        self.yields        = 0
    ...
    def growth(self, T_max, T_min, rad, CO2):
        T_mean = (T_max + T_min)/2
        # fSolar calculation
#         if self.leaf_growing:
#             fSolar = self.fSolar_max/(1 + np.exp(-0.01*(self.TT - self.I_50A)))
#             if fSolar >= self.fSolar_max*0.95: #????
#                 self.leaf_growing = False
#         else:
#             fSolar = self.fSolar_max/(1 + np.exp(0.01*(self.TT - (self.T_sum - self.I_50B))))
